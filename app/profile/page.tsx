...
export default function ProfilePage() {
  ...
  const matchedCompany = POPULAR_COMPANIES.find(c => c.name.toLowerCase() === companyName.toLowerCase());
  const companyLogoUrl = matchedCompany ? `https://www.google.com/s2/favicons?domain=${matchedCompany.domain}&sz=128` : null;
  ...
  useEffect(() => {
    ...
    if (user.companyName && companyName === "" && !window.sessionStorage.getItem('companyInit')) {
      setCompanyName(user.companyName);
      window.sessionStorage.setItem('companyInit', 'true');
    }
  }, [isAuthenticated, user, router, name, companyName]);
  ...
  const saveSection = (section: string) => {
    if (section === "info") updateUser({ name, companyName });
        ...
        <motion.div variants={fadeUp} initial="initial" animate="animate" className="bg-bg-secondary rounded-2xl shadow-sm border border-border-default overflow-hidden">
          ...
          <div className="px-6 md:px-10 pb-8 relative group/info">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end -mt-12 md:-mt-24 mb-6 gap-4">
              ...
              {/* Buttons & Company Section */}
              <div className="relative w-full md:w-auto mt-4 md:mt-0 flex flex-col items-end md:block">
                <div className="grid grid-cols-3 md:flex md:flex-wrap items-center gap-2 md:gap-3 w-full md:w-auto">
                ...
                <div className="md:absolute md:top-full md:right-0 mt-3 md:mt-2 flex flex-col items-end gap-2 w-full md:w-max z-10">
                  {editingSection === "info" && (
                    <div className="flex items-center gap-2 self-end mt-2 md:mt-0">
                      <span className="text-xs font-bold text-text-muted">Working at a company?</span>
                      <button onClick={() => setShowCompany(!showCompany)} className={`w-8 h-4 rounded-full transition-colors ${showCompany ? 'bg-brand-indigo' : 'bg-border-default'} relative outline-none`}>
                        <div className={`w-3 h-3 rounded-full bg-white absolute top-0.5 transition-transform ${showCompany ? 'translate-x-4' : 'translate-x-0.5'}`} />
                      </button>
                    </div>
                  )}
                  <AnimatePresence>
                    {showCompany && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex justify-end overflow-visible">
                        {editingSection === "info" ? (
                          <div className="relative mt-1">
                            <div className="flex items-center gap-2 bg-bg-primary border border-border-default rounded-lg px-3 py-1.5 focus-within:border-brand-indigo shadow-sm transition-colors">
                              {companyLogoUrl ? (
                                <>
                                  <img src={companyLogoUrl} alt="" onError={(e) => { e.currentTarget.style.display='none'; const next = e.currentTarget.nextElementSibling as HTMLElement; if(next) next.style.display='block'; }} className="w-4 h-4 rounded-sm object-contain" />
                                  <Building className="w-4 h-4 text-brand-indigo hidden" />
                                </>
                              ) : (
                              ...
                              <input 
                                ...
                                className="text-sm bg-transparent outline-none text-text-primary w-40 text-right" 
                              />
                            </div>
                            ...
                            <AnimatePresence>
                              {showCompanyDropdown && (
                                <motion.div 
                                  ...
                                  exit={{ opacity: 0, y: -5 }}
                                  className="absolute top-full right-0 mt-1 w-full min-w-[180px] max-h-48 overflow-y-auto bg-bg-secondary border border-border-default rounded-lg shadow-lg z-50 py-1"
                                >
                                  {POPULAR_COMPANIES.filter(c => !companyName || c.name.toLowerCase().includes(companyName.toLowerCase())).map(c => (
                                    <button 
                                      key={c.name}
                                      onClick={() => {
                                      ...
                                      className="w-full text-left px-3 py-2 text-sm text-text-primary hover:bg-bg-tertiary flex items-center gap-2 transition-colors"
                                    >
                                      <img src={`https://www.google.com/s2/favicons?domain=${c.domain}&sz=128`} onError={(e) => { e.currentTarget.style.display='none'; const next = e.currentTarget.nextElementSibling as HTMLElement; if(next) next.style.display='block'; }} alt="" className="w-4 h-4 rounded-sm object-contain" />
                                      <Building className="w-4 h-4 text-text-muted hidden" />
                                      {c.name}
                                    </button>
                                  ))}
                          ...
                          <div className="text-sm font-bold text-text-primary flex items-center gap-1.5 px-3 py-1.5 bg-bg-tertiary rounded-lg border border-border-default shadow-sm mt-1">
                            {companyLogoUrl ? (
                              <>
                                <img src={companyLogoUrl} alt="" onError={(e) => { e.currentTarget.style.display='none'; const next = e.currentTarget.nextElementSibling as HTMLElement; if(next) next.style.display='block'; }} className="w-4 h-4 rounded-sm object-contain" />
                                <Building className="w-4 h-4 text-brand-indigo hidden" />
            ...
            <div className="max-w-4xl">
              {editingSection === "info" ? (
                <div className="space-y-3 relative z-10 w-full md:w-3/4 pr-4">
